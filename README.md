...
4. **AppArmor Profile**: The custom `matrix-sandbox-profile` restricts execution paths, blocks write access to `/etc/passwd`, `/etc/shadow`, `/boot/`, denies privilege-escalation binaries, and disables all Linux kernel capabilities, mount, and raw network socket access.
